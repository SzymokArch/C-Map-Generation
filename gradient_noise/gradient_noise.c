...
float2 get_unit_vector(int y_index, int x_index, uint8_t current_octave,
		       uint64_t seed)
{
	char *hash_input;
	int hash_seed_len =
	    asprintf(&hash_input, "%i%i%hu", y_index, x_index, current_octave);
	uint64_t hash = XXH64(hash_input, hash_seed_len, seed);
	free(hash_input);
	return VECTORS[hash % VECTOR_COUNT];
}

float4 get_dots(float2 tl, float2 tr, float2 bl, float2 br, float dist_y,
		float dist_x)
{
	float4 dots;
...
float calculate_gradient_noise(double y, double x, uint8_t current_octave,
			       uint64_t seed)
{
...
float octave_gradient_noise(double y, double x, uint8_t octave_count,
			    double persistence, uint64_t seed)
{
	float noise = calculate_gradient_noise(y, x, 0, seed);
	for (uint8_t o = 0; o < octave_count; o++) {
		noise += calculate_gradient_noise(y * (2 << o), x * (2 << o), o,
						  seed) *
			 pow(persistence, o);
