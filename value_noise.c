...
void interpolate_2D(float ** map, int start_y, int start_x, int end_y, int end_x, short type){
    for (int i = 0; i <= end_y; i ++){
        map[i][0] = cos_interpolate(map[start_y][start_x], map[end_y][start_x], (double)i/(end_y));
        map[i][end_x] = cos_interpolate(map[start_y][end_x], map[end_y][end_x], (double)i/(end_y));
        for (int j = 0; j < end_x; j ++){
            switch (type){
                case 1:
                    ...
                    break;
                case 2:
                    map[i][j] = cos_interpolate(map[i][start_x], map[i][end_x], (double)j/(end_x));
                    break;
            }
        }
    }
